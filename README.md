...
- CSVは`--output`で指定した出力先へ書き出すだけで、本ツールが外部へ自動送信したりクラウドへアップロードしたりすることはありません。
...
Discord・Telegramで起動しても返信がない場合は、次の順に確認します。各手順は[Discord](#discord)・[Telegram](#telegram)のセットアップを参照してください。

Discordは通常テキストチャンネルで利用し、Bot投稿・許可外ユーザー・許可外チャンネルは無視します。Telegramは個別チャットの許可ユーザーだけを処理し、グループとBot投稿は対象外です。これらはコンソールに何も表示されず、返信がないのは正常な動作です。

1. **プロセスの起動**
   - `run_bot.py`は起動したPowerShellの前面で動き続けます。ウィンドウを閉じるか`Ctrl+C`を押すと停止します。
   - Discordは、Gateway接続の完了時、許可された投稿の受信時、返信の完了時にそれぞれ1行表示します。再接続時や重複メッセージの受信時にも表示されることがあり、ライブラリの警告が表示されることもあります。
   - Telegramは起動時・正常処理時に何も表示しません。プロンプトに戻らず、エラーも表示されていなければ動作中です。
2. **環境変数**
   - `.env.example`をコピーしたプロジェクトフォルダの`.env`に、Discordは`DISCORD_BOT_TOKEN`、`DISCORD_ALLOWED_USERS`、`DISCORD_ALLOWED_CHANNELS`、Telegramは`TELEGRAM_BOT_TOKEN`、`TELEGRAM_ALLOWED_USERS`を設定します。未設定・空欄、またはIDに数字以外が含まれると起動時にエラーで終了します。トークンが誤っている場合も、起動直後にエラーで終了します。
   - IDは半角数字、複数はカンマ区切りです。自分のユーザーID（Discordは対象チャンネルIDも）が含まれているか確認します。
   - 同じ名前のOS環境変数があると、空の値でも`.env`より優先されます。値を表示せずに有無だけ確認するには`Test-Path Env:DISCORD_BOT_TOKEN`のように実行します。
   - Discordは`requirements-discord.txt`のインストールも必要です。
3. **サービス側の状態**
   - Discord：BotがOAuth2の`bot`スコープでテストサーバーに招待され、Message Content Intentが有効か確認します。Intentが無効だと起動時にエラーで終了します。
   - Telegram：既存Webhookが残っていると、起動後すぐにエラーで終了します。WebhookはBot APIの`deleteWebhook`で解除します。同じトークンを複数のプロセスで同時に使うと、どちらかのプロセスがエラーで終了することがあります。起動後、Botとの個別チャットで`/start`を送信します。
4. **権限（Discord）**
   - 対象チャンネルでView Channel・Send Messagesが許可されているか確認します。Send Messagesがないと送信に失敗してエラーが表示され、View Channelがないと投稿を受信できず何も表示されません。
5. **コンソールのエラー**
   - トレースバックや、処理の失敗を示す行が表示されていないか確認します。
   - Discord：処理に失敗すると1行のエラーを表示し、プロセスは動作を続けます。自動再配信はしないため、原因を直して新しいメッセージで再入力してください。
   - Telegram：通信失敗などではトレースバックを表示してプロセスが終了します。接続を復旧して再起動してください。
6. **解決しない場合**
   - 起動したコンソールの出力を、起動時の表示までさかのぼって確認します。表示に時刻は付かないため、Discordでは投稿の送信直後に新しい行が表示されるかを見ると切り分けやすくなります。
   - 出力を他の人に共有する場合は、トークン・ID・メッセージ本文・PCのユーザー名を含むパスを伏せてから共有してください。GitHub Issueなどの公開の場には貼らないでください。
